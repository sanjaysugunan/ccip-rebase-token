...
contract RebaseTokenPool is TokenPool {
        ...
        returns (Pool.LockOrBurnOutV1 memory lockOrBurnOut)
    {
        ...
        uint256 interestRate = IRebaseToken(address(i_token)).getUserInterestRate(lockOrBurnIn.originalSender);
        IRebaseToken(address(i_token)).burn(address(this), lockOrBurnIn.amount);
        lockOrBurnOut = Pool.LockOrBurnOutV1({
            destTokenAddress: getRemoteToken(lockOrBurnIn.remoteChainSelector), destPoolData: abi.encode(interestRate)
        });
    }
